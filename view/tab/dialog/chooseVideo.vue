  ...
  <div>
    ...
    >
      <div slot="title" class="f-16 ver-align">
        <div class="f-16 color-4"><b>Chọn video</b></div>
      </div>
      <div style="padding: 0 20px;height: calc(100vh - 190px)">
        ...
        <div class="mt-20 ver-align" style="flex-wrap: wrap;">
            <div v-for="(item, index) in listVideo" :key="index" class="mr-10">
...
export default {
  ...
  computed: {
    ...
    listVideo() {
        return this.$store.getters.listVideo
    }
  },
  mounted() {},
  data () {
  ...
  methods: {
    ...
    handleClose () {
      this.$emit('closeChooseVideo', { type: 1 })
    },
    close () {
      this.$emit('closeChooseVideo', { type: 1 })
    },
    save () {
      this.$emit('closeChooseVideo', { type: 2, data: this.itemVideo })
    },
    catchError(error) {
